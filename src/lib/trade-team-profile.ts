...
export const getTeamTradeProfile = (
  ...
  const futurePickTolerance = Math.min(
    0.9,
    0.24 + rebuildBias * 0.24 + aggressive * 0.2 + rng() * 0.1,
  );
  const overpayForStars = Math.min(
    0.86,
    0.18 + aggressive * 0.32 + contenderBias * 0.26 + (avgRating < 74 ? 0.06 : 0),
  );
