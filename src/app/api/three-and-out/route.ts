...
export async function GET(request: Request) {
  ...
  const briefingDate = url.searchParams.get('date') ?? undefined;
  if (!TEAM_LIST.some((team) => team.abbr === teamId)) {
    return NextResponse.json({ error: 'Unknown NFL team.' }, { status: 404 });
  }
  const data = await canonicalThreeAndOut(teamId, briefingDate);
  if (!data) {
    return NextResponse.json(
      { error: 'A verified Three & Out edition is not available yet.' },
      { status: 503 },
    );
  }
