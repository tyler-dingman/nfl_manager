...
import {
  getTeamVisitState,
  updateTeamVisitState,
} from '@/server/user/content-repository';
...
export async function GET(request: NextRequest) {
  ...
  const previousVisit = await getTeamVisitState(user.id, teamId);
  const catchMeUp = buildCatchMeUp({
    previousVisit: previousVisit as {
...
export async function PUT(request: NextRequest) {
  ...
  if (!body.teamId || !body.snapshotId)
    return NextResponse.json({ error: 'teamId and snapshotId are required.' }, { status: 400 });
  return NextResponse.json({
    ok: true,
    visit: await updateTeamVisitState(user.id, body.teamId.toUpperCase(), body.snapshotId),
  });
}
