import type { UnifiedPlayer } from '@/server/data/nfl-data';
...
export const resolvePlayerRating = (player: UnifiedPlayer): number | null => {
...
const scoreSide = (groupedRatings: Map<OverviewBucket, number[]>, buckets: Set<OverviewBucket>) => {
  ...
  const capped = ratings.slice(0, 11);
  return average(capped);
};
...
export const computeTeamOverview = (players: UnifiedPlayer[]): TeamOverview => {
