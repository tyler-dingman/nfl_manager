...
test('team trade assets include all current and next year picks', () => {
  ...
  assert.deepEqual(
    [...new Set(kcAssets.draftPicks.map((pick) => pick.year))].sort(),
    [2026, 2027],
  );
  assert.deepEqual(
...
test('transferred picks move to the acquiring team asset list', () => {
  ...
  assert.equal(kcAssets.draftPicks.some((pick) => pick.originalTeamAbbr === 'KC' && pick.year === 2026 && pick.round === 2), false);
  assert.equal(seaAssets.draftPicks.some((pick) => pick.originalTeamAbbr === 'KC' && pick.year === 2026 && pick.round === 2), true);
});
