...
export async function importNflverseHistory(options: Options) {
  ...
  for (const game of gamesRaw) {
    ...
    await db`INSERT INTO historical_games(season,week,season_type,provider,provider_game_id,game_date,kickoff_at,home_team_id,away_team_id,home_score,away_score,status) VALUES(${Number(game.season)},${Number(game.week)},${game.game_type},'NFLVERSE',${game.game_id},${game.gameday},${kickoff},${team(game.home_team)},${team(game.away_team)},${integerOrNull(game.home_score)},${integerOrNull(game.away_score)},${game.home_score && game.away_score ? 'FINAL' : 'SCHEDULED'}) ON CONFLICT(season,season_type,week,home_team_id,away_team_id) DO UPDATE SET provider=EXCLUDED.provider,provider_game_id=EXCLUDED.provider_game_id,game_date=EXCLUDED.game_date,kickoff_at=CASE WHEN historical_games.espn_event_id IS NULL THEN EXCLUDED.kickoff_at ELSE historical_games.kickoff_at END,home_score=EXCLUDED.home_score,away_score=EXCLUDED.away_score,status=CASE WHEN EXCLUDED.home_score IS NOT NULL AND EXCLUDED.away_score IS NOT NULL THEN 'FINAL' ELSE historical_games.status END,updated_at=now()`;
  }
