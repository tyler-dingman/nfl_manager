...
const rows = [
  ...
  { label: 'Favorite team', href: '/team-select' as const },
  { label: 'The Crew', href: '/crew' as const },
  { label: 'Film Room', href: '/film-room' as const },
  { label: 'Front Office', href: '/front-office' as const },
  { label: 'Merch', href: '/merch' as const },
  { label: 'My Orders', href: '/orders' as const },
  { label: 'Notification Center', href: '/notifications' as const },
  { label: 'Notification preferences', href: '/notification-settings' as const },
  { label: 'Saved content', href: '/saved' as const },
  { label: 'The Locker', href: '/rewards' as const },
];
    ...
    <View style={s.page}>
      ...
      <View style={[s.card, { backgroundColor: theme.dark }]}>
        {user?.avatarUrl ? (
          ...
          <View style={[s.avatarFallback, { backgroundColor: theme.primary }]}>
            <Text style={[s.avatarLetter, { color: theme.light }]}>
      ...
      {rows.map((row, i) => (
        <Pressable key={row.label} style={s.row} onPress={() => router.push(row.href as Href)}>
      ...
      <Pressable
        style={[s.logout, { borderColor: theme.primary }]}
        disabled={busy}
        onPress={() => void logout()}
      >
        <Text style={[s.logoutText, { color: theme.primary }]}>
...
const s = StyleSheet.create({
  ...
  avatar: { width: 54, height: 54, borderRadius: 27, marginBottom: 14 },
  avatarFallback: {
    width: 54,
    height: 54,
    borderRadius: 27,
  ...
  avatarLetter: { fontSize: 23, fontWeight: '900' },
  row: {
